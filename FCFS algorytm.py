...
if decyzja == 1:
    ...
    print(procesy)  # wyswietl wszystkie procesy
    # struktora listy procesow - [przyjscie][trwanie][zakonczenie][oczekiwanie]
# Algorytm FCFS
czas_w_petli = 0
while True:     # petla ktora wykonuje sie az do zakonczenia wszystkich procesow
    for i in range(len(procesy)):
        if procesy[i][0] == czas_w_petli:
            x = [procesy[i][0], procesy[i][1], procesy[i][2], procesy[i][3]]
            aktualne.append(x)      # dodajemy do listy procesy ktore juz się rozpoczely tworzac kolejke
    czas_w_petli += 1
...
print("Dla algorytmu FCFS, sredni czas oczekiwania wynosi:", suma/n)
